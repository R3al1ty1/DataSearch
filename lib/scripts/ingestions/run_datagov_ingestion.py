...
async def test_datagov_ingestion():
    ...
    logger.info("=== Testing Data.gov Dataset Ingestion ===")
    logger.info(f"Database URL: {container.settings.SQLALCHEMY_DATABASE_URI}")

    limit = int(os.getenv("DATAGOV_LIMIT", "10"))
    days_back = int(os.getenv("DATAGOV_DAYS_BACK", "0"))
    batch_size = int(os.getenv("DATAGOV_BATCH_SIZE", "100"))
    query = os.getenv("DATAGOV_QUERY", "")
    min_harvested = (
    ...
    async with container.uow() as uow:
        try:
            count_before = await uow.session.execute(
                select(func.count(Dataset.id)).where(Dataset.source_name == "datagov")
            )
            total_before = count_before.scalar_one()
            logger.info(f"Data.gov datasets before: {total_before}")

            logger.info(
                "Fetching Data.gov datasets "
                f"(limit={limit}, days_back={days_back}, "
                f"batch_size={batch_size}, query={query!r})..."
            )
            fetched, saved = await container.datagov_processor.fetch_and_store(
                uow,
                limit=limit,
            ...
            logger.info(f"Data.gov datasets after: {total_after}")

            result = await uow.session.execute(
                select(Dataset)
                .where(Dataset.source_name == "datagov")
            ...
            for i, dataset in enumerate(latest_datasets, 1):
                ...
                logger.info(
                    f"  Description: "
                    f"{dataset.description[:100] if dataset.description else 'N/A'}..."
                )
                ...
                logger.info(
                    f"  Column Names: "
                    f"{dataset.column_names[:5] if dataset.column_names else []}"
                )
                ...
                logger.info(
                    f"  Source Meta Keys: "
                    f"{list(dataset.source_meta.keys()) if dataset.source_meta else 'N/A'}"
                )
...
if __name__ == "__main__":
    asyncio.run(test_datagov_ingestion())
